...
export async function createProduct(formData: FormData) {
  ...
  if (!values.name) redirect("/produtor/produtos/novo?error=" + encodeURIComponent("Informe o nome do produto"));

  const { error } = await supabase.from("products").insert({ ...values, producer_id: user.id });
  if (error) redirect("/produtor/produtos/novo?error=" + encodeURIComponent(error.message));

  revalidatePath("/produtor"); revalidatePath("/produtor/produtos");
...
export async function updateProduct(id: string, formData: FormData) {
  ...
  const values = parseForm(formData);
  if (!values.name) redirect(`/produtor/produtos/${id}?error=` + encodeURIComponent("Informe o nome do produto"));

  const { error } = await supabase.from("products").update(values).eq("id", id).eq("producer_id", user.id);
  if (error) redirect(`/produtor/produtos/${id}?error=` + encodeURIComponent(error.message));
