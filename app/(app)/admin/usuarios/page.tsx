...
export default async function UsuariosPage() {
  ...
  const { data } = await supabase
    .from("profiles")
    .select("id, role, full_name, city, state, verification_status, created_at")
          ...
          <tbody className="divide-y divide-campo-border glass">
            {rows.map((u) => (
              <tr key={u.id} className="transition hover:bg-campo-surface2">
                ...
                <td className="px-4 py-3 text-stone-500">{new Date(u.created_at).toLocaleDateString("pt-BR")}</td>
              </tr>
            ))}
          </tbody>
        </table>
