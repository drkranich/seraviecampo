...
export default async function FinanceiroParceiroPage({
  ...
  const { data: acc } = await supabase
    .from("profiles").select("stripe_account_id, stripe_charges_enabled, stripe_account_version, stripe_account_status, stripe_transfers_status, stripe_requirements_due, stripe_requirements_past_due, payout_mode, currency").eq("id", user.id).single();
