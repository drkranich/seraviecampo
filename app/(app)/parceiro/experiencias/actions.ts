...
export async function createExperience(formData: FormData) {
  ...
  if (!values.title) redirect("/parceiro/experiencias/nova?error=" + encodeURIComponent("Informe o título da experiência"));
  ...
  if (error) redirect("/parceiro/experiencias/nova?error=" + encodeURIComponent(error.message));

  revalidatePath("/parceiro/experiencias");
  redirect("/parceiro/experiencias");
}
...
export async function updateExperience(id: string, formData: FormData) {
  ...
  const values = parseForm(formData);
  if (!values.title) redirect(`/parceiro/experiencias/${id}?error=` + encodeURIComponent("Informe o título da experiência"));

  const { error } = await supabase.from("experiences")
    .update({ ...values, updated_at: new Date().toISOString() })
    .eq("id", id).eq("producer_id", user.id);
  if (error) redirect(`/parceiro/experiencias/${id}?error=` + encodeURIComponent(error.message));
...
export async function setBookingStatus(id: string, status: ExperienceStatus) {
  ...
  await supabase.from("experience_bookings")
    ...
    .eq("id", id).eq("producer_id", user.id);
  revalidatePath("/parceiro/experiencias");
  redirect("/parceiro/experiencias?ok=1");
}
